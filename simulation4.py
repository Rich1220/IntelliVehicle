...
class OvertakingDecisionMaker:
    ...
    def solve_stackelberg_equilibrium(self, payoff_matrix: dict) -> Tuple[GameAction, GameAction]:
        """
        對向車為領導者（Leader），主車為跟隨者（Follower）。
        對每個 leader 的策略，找出 follower 的最佳反應，然後選 leader payoff 最大的組合。
        """
        Y, N = GameAction.YIELD, GameAction.NOT_YIELD
        actions = [(Y, Y), (Y, N), (N, Y), (N, N)]

        best_leader_payoff = -float('inf')
        best_combination = (Y, Y)

        for leader_action in [Y, N]:
            # 找出 follower（主車）對這個 leader 策略的最佳反應
            best_follower_action = Y
            best_follower_payoff = -float('inf')

            for follower_action in [Y, N]:
                key = f"{follower_action.name[0]}{leader_action.name[0]}"
                ego_payoff = payoff_matrix[key][0].total_payoff
                if ego_payoff > best_follower_payoff:
                    best_follower_payoff = ego_payoff
                    best_follower_action = follower_action

            # leader 根據 follower 的反應，計算自己的 payoff
            key = f"{best_follower_action.name[0]}{leader_action.name[0]}"
            leader_payoff = payoff_matrix[key][1].total_payoff

            if leader_payoff > best_leader_payoff:
                best_leader_payoff = leader_payoff
                best_combination = (best_follower_action, leader_action)

        return best_combination  # (ego_action, oncoming_action)

    def solve_nash_equilibrium(self, payoff_matrix) -> Tuple[GameAction, GameAction]:
        """
        兩車博弈：先找純策略 Nash；若無，再按照主車偏好 + 風險偏好選行動
        """
        # 方便索引
        Y, N = GameAction.YIELD, GameAction.NOT_YIELD
        s = [Y, N]
        ...
        # 1. 組 2×2 payoff 矩陣
        ego_mat = np.array([
            [payoff_matrix['YY'][0].total_payoff, payoff_matrix['YN'][0].total_payoff],
            [payoff_matrix['NY'][0].total_payoff, payoff_matrix['NN'][0].total_payoff],
        ])
        on_mat = np.array([
            [payoff_matrix['YY'][1].total_payoff, payoff_matrix['YN'][1].total_payoff],
            [payoff_matrix['NY'][1].total_payoff, payoff_matrix['NN'][1].total_payoff],
        ])

        # 2. 各自的最佳回應 (Best Response)
        ego_br = (ego_mat == ego_mat.max(axis=0))          # col-wise max
        on_br  = (on_mat  == on_mat.max(axis=1, keepdims=True))  # row-wise max

        # 3. 檢查是否有同時為雙方最佳回應的位置 → Nash
        for i in range(2):
            for j in range(2):
                if ego_br[i, j] and on_br[i, j]:
                    return s[i], s[j]

        # 4. 若無純策略 Nash → 採用「主車激進」啟發式
        #    （在此處可加你原本的偏好係數，或任何自訂 heuristic）
        #    ── 以下沿用你舊邏輯，但寫得更易懂 ──
        ego_ny = ego_mat[1, 0] + 0.25        # Ego 超車、對向禮讓
        ego_nn = ego_mat[1, 1] + 0.15        # 雙方都不讓
        ego_yy = ego_mat[0, 0]
        ego_yn = ego_mat[0, 1]

        # prefer NOT_YIELD unless payoff 明顯差太多
        if ego_ny >= 0.8 * ego_yy:
            return N, Y
        if ego_nn >= 0.8 * ego_yn:
            return N, N
        return Y, Y  # 保守 fallback
    ...
    def make_overtaking_decision(self, ego_state: VehicleState, 
        ...
        s_conflict = conflict_area.position
        s_oncoming = closest_oncoming.position
        v_oncoming = closest_oncoming.speed

        if s_oncoming > s_conflict:  # 確保是對向車還在衝突區之後
            amin_required = - (v_oncoming ** 2) / (2 * (s_oncoming - s_conflict + 1e-5))  # 加1e-5防除零
            if amin_required < -5.0:  # 安全門檻，可以依場景調整
                print("[SAFETY OVERRIDE] 對向車無法在最大減速度下停車，強制主車讓行。")
                return GameAction.YIELD

        # Game-theoretic decision
        payoff_matrix = self.calculate_game_payoffs(...)
        ego_action, _ = self.solve_stackelberg_equilibrium(payoff_matrix)
        return ego_action
...
<routes xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="http://sumo.dlr.de/xsd/routes_file.xsd">
    ...
    <vehicle id="oncoming1" type="oncoming_type" route="route_westbound" depart="0" departPos="800" departSpeed="20">
        <param key="lcStrategic" value="0.0"/>
    </vehicle>
